...
PUSHER_APP_ID = os.getenv("PUSHER_APP_ID", "1234567")
...
@app.route('/mandoob/<shopify_order_id>')
def mandoob_page(shopify_order_id):
...
def create_checkout_session():
    ...
    try:
        session = stripe.checkout.Session.create(
            ...
            line_items=[{'price_data': {'currency': 'egp', 'product_data': {'name': f'PayDOD Order #{shopify_order_id}'}, 'unit_amount': amount}, 'quantity': 1}],
            mode='payment',
        ...
        with get_db() as db:
            db.execute('INSERT OR REPLACE INTO orders (shopify_order_id, amount, status) VALUES (?, ?, ?)', (shopify_order_id, amount / 100, 'PENDING'))
            db.commit()
...
def stripe_webhook():
    ...
    try:
        event = stripe.Webhook.construct_event(payload, sig_header, STRIPE_WEBHOOK_SECRET) if STRIPE_WEBHOOK_SECRET != "whsec_YOUR_WEBHOOK_SECRET" else request.get_json()
    except Exception as e:
        return jsonify({'status': 'invalid payload', 'error': str(e)}), 400

    if event['type'] == 'checkout.session.completed':
        shopify_order_id = event['data']['object'].get('metadata', {}).get('shopify_order_id')
        if shopify_order_id:
            with get_db() as db:
                db.execute('UPDATE orders SET status = ? WHERE shopify_order_id = ?', ('PAID', shopify_order_id))
                db.commit()
            update_shopify_order(shopify_order_id)
            pusher_client.trigger('paydod-channel', 'payment-success', {'shopify_order_id': shopify_order_id, 'status': 'PAID', 'message': 'تم الدفع بنجاح!'})
...
def update_shopify_order(shopify_order_id):
    ...
    data = {"transaction": {"kind": "capture", "status": "success", "amount": "10.00"}}
    try:
